...
def precision(preds, target):
    ...
    return metrics.precision(
        ...
        target['y'].reshape(-1).masked_select(mask).to(int), average='micro', task='binary')
...
def recall(preds, target):
    ...
    return metrics.recall(
        f.sigmoid(preds.reshape(-1).masked_select(mask)), target['y'].reshape(-1).masked_select(mask).to(int), average='micro', task='binary')
