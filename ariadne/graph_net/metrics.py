...
@gin.configurable('graph_net.precision', whitelist=[])
def precision(preds, target):
    return metrics.precision(
        preds > 0.5, target, reduction='none', num_classes=2)[1]

@gin.configurable('graph_net.recall', whitelist=[])
def recall(preds, target):
    return metrics.recall(
        preds > 0.5, target, reduction='none', num_classes=2)[1]

@gin.configurable('graph_net.f1_score', whitelist=[])
def f1_score(preds, target):
    return metrics.f1_score(
        preds > 0.5, target, reduction='none', num_classes=2)[1]

@gin.configurable('graph_net.accuracy', whitelist=[])
