...
from ariadne.graph_net.graph_utils.graph_prepare_utils import get_plain_graph, get_plain_graph_new, \
    apply_edge_rescriction_new, construct_output_graph_new, \
...
# gin.bind_parameter('apply_edge_rescriction_new.restrictions_0',(-10,10))
# gin.bind_parameter('apply_edge_rescriction_new.restrictions_1',(-10,10))
...
class GraphModelLoader(IModelLoader):
    def __call__(self):
        ...
        gin.bind_parameter('GraphNet_v1.input_dim', 5)  # 5
        ...
        path_g = 'lightning_logs/GraphNet_v1/version_197/epoch=1-step=4999.ckpt'  # _fakes
...
def construct_graph_with_indices_torchscript(graph):
    return GraphWithIndices(graph.X.cpu().numpy(), graph.Ri.cpu().numpy(), graph.Ro.cpu().numpy(),
                            graph.y.cpu().numpy(), graph.edges.cpu().numpy())


# process_single = torch.jit.load('scriptmodule.pt')


from torchscript_rd import ProcessSingleEvent

loaded = torch.jit.load('E:/rd.pt')
process_single = None
...
def get_graph_torchscipt(event, node_indices, edge_indices,super_indices):
    global process_single
    if process_single == None:
        process_single = ProcessSingleEvent('cpu', node_indices, edge_indices,super_indices)
...
def get_graph(event_data, column_index,*_):
    ...
    # print( event[event['index_old']==7094. ], event[event['index_old']==7095. ] )
    device = 'cpu'
    ...
    nodes, edges, node_indices, edge_indices = get_plain_graph_new(event_data, column_index, device=device,
                                                                   suffixes=suff_df, compute_is_true_track=False)
    ...
    if mode == 'reversed':
        ...
        edges, edge_indices = get_edges_from_supernodes_new(edges, edge_indices,device)

    # ev_id = event.event.values[0]
    # weight = edges[edges[:, edge_indices['is_true']].bool() == True][:, edge_indices['weight']]
    # weight = edges[:, edge_indices['weight']]
    ...
    return graph_with_inds, nodes[:, [node_indices['index_old_p'], node_indices['index_old_c'], node_indices[
        'index']]]  # edges[:, [edge_indices[x] for x in ['from_ind', 'cur_ind', 'to_ind' ] ] ]
...
def eval_event(tgt_graph, model_g, ev_id):
    ...
    tgt_graph, real_edges = tgt_graph
    ...
    # nodes = pd.DataFrame( real_edges[y_pred].numpy() )

    adj = torch.t(
        torch.matmul(tgt_graph.Ri[:, y_pred].float(), torch.t(tgt_graph.Ro[:, y_pred].float())))  # .to(torch.float32)
    ...
    for track in tracks:
        tracks_.append( real_index(track) )
    ...
    adj_to_34 = torch.matrix_power(adj.cuda(), power)
    ...
    v_all = np.empty((adj_to_34.sum().int(), N_STATIONS), dtype=np.int32)

    count = 0
    real_edges = real_edges.cpu().numpy()

    for first, last in start_end_idx:

        #if adj_to_34[first,last] >10:
        #    continue

        paths = bfs__(graph_dict, tgt_graph.index[first].item(),
                      tgt_graph.index[last].item())  # dfs(graph_dict, [hit],[],ev_id=ev_id)
        if mode == 'reversed':

            for i in range(len(paths)):
                inds = np.nonzero(real_edges[:, 2] == paths[i][..., None])[
                    1]  # real_edges[real_edges[:,2] == paths[i] ]
                edges = real_edges[inds]
                v_all[last_row:last_row + 1, :] = np.append(edges[:, 0][::-1], edges[:, 1][0])
                last_row += 1
            count += paths.shape[0]
    ...
    # end__ = timer()
    # print('searching for tracks_new', end-start,'searching for tracks_old', end__-start__,'evaluation',end_-start_)
    eval_df = pd.DataFrame(v_all[0:count, :], columns=[f"hit_id_{n}" for n in range(1, N_STATIONS + 1)])
...
model_results = evaluator.run_model(get_graph_torchscipt, eval_event)
