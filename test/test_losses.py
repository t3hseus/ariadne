...
class EllipseSquareLossTest(TestCase):
    ...
    def test_output(self):
        ...
        expected_outputs = torch.tensor([0., 0., 1., 0.27, 5.])
        outputs = self._loss_func(preds)
        torch.testing.assert_allclose(outputs, expected_outputs)
...
class PointInEllipseLossTest(TestCase):
    ...
    def test_inputs_with_shapes_diff(self):
    ...
    def test_preds_incorrect_shape(self):
        target = torch.zeros((2, 2))
        preds = torch.zeros((2, 3))
        with self.assertRaisesRegex(ValueError, 'Prediction must be 4-dimensional'):
            self._loss_func(preds, target)

    def test_target_incorrect_shape(self):
